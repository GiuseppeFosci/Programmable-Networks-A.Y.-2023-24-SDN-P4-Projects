import os
import time
from scapy.all import sr1, send, IP, ICMP, conf
...
def send_icmp_ping_no_reply(target_ip):
    ...
    """
    pkt = IP(dst=target_ip)/ICMP()
    # send() invia il pacchetto senza attendere il reply
...
def main():
    while True:
        for intf in INTERFACES:
            ...
            # Invia un singolo ping al gateway
            send_icmp_ping_no_reply(TARGET_IP)
            # Aspetto 10 secondi prima di passare all'interfaccia successiva
            time.sleep(10)
